...
const activityModule = {
  ...
  mutations: {
    ...
    setActivityPending(state, pending) {
  ...
  actions: {
    ...
    async getDetail({ commit }, { id }) {
      ...
      item.end_join_time = moment(item.end_join_time * 1000).format('YYYY-MM-DD');
            ...
            wx.chooseWXPay({
              ...
              success() {
                ...
                setTimeout(() => {
                  router.replace({
                    name: 'Ticket-detail',
                    params: { id: res.data.id, type: 0 },
                  });
                }, 20);
              },
              fail(error) {
                alert('支付失败', error); // eslint-disable-line no-alert
              },
            });
          }
        } else {
